...
class GReSTIRIntegrator(mi.SamplingIntegrator):
    ...
    def direct(
        ...
        ds, emitter_value = scene.sample_emitter_direction(
            si, sampler.next_2d(), active=active
        )
        ds = ds  # type: mi.DirectionSample3f

        bsdf: mi.BSDF = si.bsdf(ray)
        bsdf_weight = bsdf.eval(mi.BSDFContext(), si, si.to_local(ds.d), active)

        return emitter_value * bsdf_weight
    ...
    def sample(
        ...
        if count > 0:
            Li, _, _ = self.sample(scene, sampler, ray, active=active, count=count - 1)
        else:
            Li = self.direct(scene, sampler, ray, active)
            # Li = self.sample_ray(
            #     scene, sampler, si.spawn_ray(si.to_world(wo)), active=active
            # )

        w = dr.select(dr.eq(p, 0), 0, p_hat(Li) / p)
        ...
        sample_update.s.wo = wo
        sample_update.s.Li = Li
        sample_update.w = w
        sample_update.r = sampler.next_1d()

        R = dr.gather(restir.Reservoir, self.temporal, reservoir_idx, active)
        res = update(R, sample_update)
        dr.scatter(self.temporal, res, reservoir_idx, active)
