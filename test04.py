...
class GReSTIRIntegrator(mi.SamplingIntegrator):
    ...
    def sample(
        ...
        si, _, _ = self.first_non_specular_or_null_si(scene, si, sampler)
        # si = self.first_non_specular(scene, sampler, ray, active)
        # print(f"{si=}")
        active &= si.is_valid()
        bsdf: mi.BSDF = si.bsdf(ray)
...
if __name__ == "__main__":
    ...
    scene = mi.load_file(
        "./data/scenes/cornell-box-specular/scene.xml"
    )  # type: mi.Scene
