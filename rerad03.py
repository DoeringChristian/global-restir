...
class GReSTIRIntegrator(mi.SamplingIntegrator):
    ...
    def generate_sample(self, scene: mi.Scene, sampler: mi.Sampler) -> RestirSample:
        ...
        Li, _, _ = self.sample(
            scene, sampler, si.spawn_ray(si.to_world(wi)), None, mi.Bool(True)
        )
    ...
    def temporal_resampling(self, scene: mi.Scene):
        sampler = mi.load_dict({"type": "independent"})  # type: mi.Sampler
        sampler.seed(self.n, self.n_reservoirs)
        ...
        self.reservoirs = Rnew

        self.grid = HashGrid(
            self.reservoirs.z.x0, 100, self.n_reservoirs
        )  # search radius has to be at least two times the cell width
        self.n += 1
